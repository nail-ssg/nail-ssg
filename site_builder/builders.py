...
def build_static(site_builder: SiteBuilder):
    dist = str(site_builder.config('core.dist', _default_distributive_folder))
    rmtree(dist, True)
    sleep(1)
    static_path = os.sep.join([site_builder.config('core.src'), site_builder.config('static.folder', 'static')])
...
def build_pages(site_builder: SiteBuilder):
    ...
    for page in site_builder.allData['allPages']:
        ...
        src = site_builder.config('core.src')
        src_filename = os.sep.join([src, page['$filePath']])
        p = os.sep.join([src, site_builder.config('pages.folder')])
        rel_path = os.path.relpath(src_filename, p)
        dist = site_builder.config('core.dist', _default_distributive_folder)
